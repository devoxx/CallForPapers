...
object ConferenceDescriptor {
  ...
  object ConferenceSlots {
    ...
    // DEVOXX DAYS
    def wednesdaySchedule: List[Slot] = {
      alldb.filter(x=>x.day.equalsIgnoreCase("wednesday"))
    }

    def thursdaySchedule: List[Slot] = {
      alldb.filter(x=>x.day.equalsIgnoreCase("thursday"))
    }

    def fridaySchedule: List[Slot] = {
      alldb.filter(x=>x.day.equalsIgnoreCase("friday")) }
